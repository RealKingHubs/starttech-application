...
# Fail fast if no instances are registered in SSM
if [ -z "$INSTANCE_IDS" ]; then
    echo "ERROR: No EC2 instances found registered in SSM Parameter Store/Managed Instances."
    exit 1
fi
...
aws ssm send-command \
  ...
  --parameters "commands=[
    ...
    \"docker run -d --name backend -p 8080:8080 \\
      ...
      -e ORIGINS_STRING='http://localhost:5173,http://amazonaws.com' \\
      -e ENABLE_CACHE=true \\
